\begin{enumerate}[resume*]
    ...
    \item \task{ММО, 2013, 11.3}{Четырёхугольник $ABCD$ такой, что $AB = BC$ и $AD = DC$. Точки $K$, $L$ и $M$ -- середины отрезков $AB$, $CD$ и $AC$ соответственно. Перпендикуляр, проведённый из точки $A$ к прямой $BC$, пересекается с перпендикуляром, проведённым из точки $C$ к прямой $AD$, в точке $T$. Докажите, что прямые $KL \perp TM$.} 
